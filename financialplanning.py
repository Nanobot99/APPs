...
def montecarlo_accumulate_and_decumulate(init_portfolio, yearly_contribution, years_accum, years_decum,
                                         ...
                                         yearly_contribution_schedule=None, cashflow_schedule=None):
    """
    Monte Carlo simulation that supports:
    - yearly_contribution_schedule: optional list of length >= years_accum where each element is the contribution for that accumulation year.
      If not provided, uses constant yearly_contribution.
    - cashflow_schedule: optional dict {year_index: cashflow_amount} applied at that accumulation year (positive for inflow, negative for outflow).
      For example, downpayments can be represented as negative amounts at the purchase year.
    """
    ...
    for s in range(sims):
        ...
        for y in range(years_accum):
            ...
            pv = pv * (1 + ret)
            # contribution (allow a schedule)
            if yearly_contribution_schedule is not None and y < len(yearly_contribution_schedule):
                contrib = yearly_contribution_schedule[y]
            else:
                contrib = yearly_contribution
            pv = pv + contrib
            # apply one-time cashflows (e.g., downpayment) at this year after contribution
            if cashflow_schedule is not None and y in cashflow_schedule:
                pv += cashflow_schedule[y]
            path.append(pv)
...
# New helper to compute annual EMI given loan amount, rate and tenure
def compute_annual_emi(loan_amount, annual_rate, tenure_years):
...
with st.sidebar:
    ...
    # NEW: SIP duration input (years)
    sip_years = st.number_input("SIP duration (years)", 0, 65, max(0, retirement_age - age), step=1)
    ...
    downpayment_share = st.slider("Downpayment share for loans (%)", 0.0, 100.0, 20.0)/100.0
...
# SIP & MF projections (respect SIP duration)
sip_years_effective = max(0, min(years_left, int(sip_years)))
sip_fv = sip_future_value(sip_monthly, sip_expected_return, sip_years_effective)
...
if years_left > 0:
    investment_income_est = (sip_fv + mf_future + total_stock_future - (sip_monthly*12*sip_years_effective + mf_current + total_stock_current)) / max(1, years_left)
else:
...
# NEW: include SIP contribution as expense (current annual SIP if SIP is active)
sip_annual_now = sip_monthly * 12.0 if sip_years > 0 else 0.0

# NEW: travel costs now
travel_annual_now = (domestic_trips_per_year * avg_domestic_trip_cost) + (international_trips_per_year * avg_international_trip_cost)

annual_consumption_now = medical_cost_now + grocery_annual + food_other_annual + househelp_annual + emi_annual + utilities_annual + sip_annual_now + travel_annual_now
...
# NEW: calculate car & house purchase prices at their purchase years (inflated)
car_price_at_purchase = inflate_amount(car_price_input, general_inflation, car_purchase_in_years)
house_price_at_purchase = inflate_amount(house_price_input, general_inflation, house_purchase_in_years)

# NEW: compute loan EMIs at purchase-time nominal values (these EMIs are in purchase-year currency)
car_loan_amount = max(0.0, car_price_at_purchase * (1.0 - downpayment_share))
house_loan_amount = max(0.0, house_price_at_purchase * (1.0 - downpayment_share))
car_emi_annual_at_purchase = compute_annual_emi(car_loan_amount, car_interest, car_loan_years)
house_emi_annual_at_purchase = compute_annual_emi(house_loan_amount, house_interest, house_loan_years)

# For "at retirement" we need to include EMIs only if loan active at that future year
def emi_active_at_future(purchase_after_years, loan_tenure_years, years_future):
    return (years_future >= purchase_after_years) and (years_future < (purchase_after_years + loan_tenure_years))

car_emi_ret = car_emi_annual_at_purchase if emi_active_at_future(car_purchase_in_years, car_loan_years, years_to_ret) else 0.0
house_emi_ret = house_emi_annual_at_purchase if emi_active_at_future(house_purchase_in_years, house_loan_years, years_to_ret) else 0.0

# NEW: travel at retirement (inflate current travel)
travel_annual_ret = inflate_amount(travel_annual_now, general_inflation, years_to_ret)

# NEW: SIP expense at retirement: only if SIP still active by then (we treat SIP as being paid for sip_years)
sip_annual_ret = sip_monthly * 12.0 if sip_years > years_to_ret else 0.0
if sip_annual_ret > 0:
    sip_annual_ret = inflate_amount(sip_annual_ret, general_inflation, years_to_ret)

annual_consumption_at_retirement = (medical_cost_ret + grocery_annual_ret + food_other_annual_ret +
                                   househelp_annual_ret + emi_annual_ret + utilities_annual_ret +
                                   travel_annual_ret + sip_annual_ret + car_emi_ret + house_emi_ret)
...
def projected_consumption_in_years(years_future, family_size_future):
    ...
    # travel projection
    travel_future = inflate_amount(travel_annual_now, general_inflation, years_future)

    # sip projection: if SIP is still scheduled at that future year (years_future < sip_years), include it; otherwise 0
    sip_future = 0.0
    if sip_years > 0 and years_future < sip_years:
        sip_future = inflate_amount(sip_monthly * 12.0, general_inflation, years_future)

    # car & house EMI if active at that future year
    car_emi_future = 0.0
    if emi_active_at_future(car_purchase_in_years, car_loan_years, years_future):
        # EMI was computed at purchase-time currency; but we want value in the years_future currency.
        # The EMI at purchase is based on purchase-time price (inflated to that year). If years_future == purchase year, no extra inflation.
        # If years_future > purchase year and loan still active, EMI should remain same nominally (we'll not re-inflate).
        car_emi_future = car_emi_annual_at_purchase

    house_emi_future = 0.0
    if emi_active_at_future(house_purchase_in_years, house_loan_years, years_future):
        house_emi_future = house_emi_annual_at_purchase

    total_future = (medical_future + grocery_future + food_other_future + hh_future + emi_future + util_future +
                    travel_future + sip_future + car_emi_future + house_emi_future)
    return {
        ...
        "sip": sip_future,
        "car_emi": car_emi_future,
        "house_emi": house_emi_future,
        "total": total_future
    }
...
if total_current <= 0:
    ...
    w_sip = (sip_monthly*12*sip_years_effective) / total_current if total_current>0 else 0
...
# Build yearly contribution schedule and cashflow schedule to account for car/house purchases & EMIs
years_accum = years_left
# base yearly contribution is net_surplus_now (current surplus after consumption and SIP)
base_yearly_contrib = max(0.0, net_surplus_now)

# Build schedule (list) of yearly contributions for each accumulation year
yearly_contrib_schedule = []
for y in range(years_accum):
    contrib = base_yearly_contrib
    # if car EMI active in this accumulation year subtract EMI
    if emi_active_at_future(car_purchase_in_years, car_loan_years, y):
        contrib = max(0.0, contrib - car_emi_annual_at_purchase)
    # if house EMI active in this accumulation year subtract EMI
    if emi_active_at_future(house_purchase_in_years, house_loan_years, y):
        contrib = max(0.0, contrib - house_emi_annual_at_purchase)
    # if SIP scheduled to stop before this year, keep contrib same (SIP already included in consumption)
    yearly_contrib_schedule.append(contrib)

# Build cashflow schedule for one-time downpayments (negative amounts)
cashflow_schedule = {}
# Car downpayment occurs at car_purchase_in_years (if within accumulation horizon)
if 0 <= car_purchase_in_years < years_accum:
    car_downpayment = -1.0 * (car_price_at_purchase * downpayment_share)
    cashflow_schedule[int(car_purchase_in_years)] = cashflow_schedule.get(int(car_purchase_in_years), 0.0) + car_downpayment
# House downpayment occurs at house_purchase_in_years (if within accumulation horizon)
if 0 <= house_purchase_in_years < years_accum:
...
if st.button("Run Monte Carlo simulation (accumulate + decumulate)"):
    ...
    yearly_contrib = base_yearly_contrib if base_yearly_contrib > 0 else 0.0

    # Use the schedule and cashflows constructed earlier
    success_rate, finals, percentiles, full_paths = montecarlo_accumulate_and_decumulate(
        ...
        sims=int(sims),
        seed=42,
        yearly_contribution_schedule=yearly_contrib_schedule,
...
with tabs[0]:
    ...
    st.write(f"- EMIs (current): {currency_symbol}{emi_annual:,.0f}")
    st.write(f"- Utilities: {currency_symbol}{utilities_annual:,.0f}")
    st.write(f"- SIP contributions (annual): {currency_symbol}{sip_annual_now:,.0f}")
    st.write(f"- Travel (annual): {currency_symbol}{travel_annual_now:,.0f}")
...
with tabs[1]:
    ...
    st.write(f"- EMIs (ret existing EMIs): {currency_symbol}{emi_annual_ret:,.0f}")
    st.write(f"- Utilities (ret): {currency_symbol}{utilities_annual_ret:,.0f}")
    st.write(f"- SIP contributions at ret (if active): {currency_symbol}{sip_annual_ret:,.0f}")
    st.write(f"- Travel (ret): {currency_symbol}{travel_annual_ret:,.0f}")
    st.write(f"- Car EMI (if active at ret): {currency_symbol}{car_emi_ret:,.0f}")
    st.write(f"- House EMI (if active at ret): {currency_symbol}{house_emi_ret:,.0f}")
...
with tabs[2]:
    ...
    st.write(f"- Travel (15y): {currency_symbol}{proj_15y['travel']:,.0f}")
    st.write(f"- SIP at 15y (if active): {currency_symbol}{proj_15y['sip']:,.0f}")
    st.write(f"- Car EMI at 15y (if active): {currency_symbol}{proj_15y['car_emi']:,.0f}")
    st.write(f"- House EMI at 15y (if active): {currency_symbol}{proj_15y['house_emi']:,.0f}")
...
with tabs[3]:
    ...
    st.write(f"- Travel (30y): {currency_symbol}{proj_30y['travel']:,.0f}")
    st.write(f"- SIP at 30y (if active): {currency_symbol}{proj_30y['sip']:,.0f}")

with tabs[4]:
    st.write("### Investments & projections")
    st.write(f"- Current investments total: {currency_symbol}{current_investments:,.0f}")
    st.write(f"- SIP future value (to retirement): {currency_symbol}{sip_fv:,.0f} (SIP duration used: {sip_years_effective} years)")
